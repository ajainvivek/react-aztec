...
const validation = {
  ...
  isNumeric(values, value) {
    ...
    return validation.matchRegexp(values, value, /^[-+]?(?:\d*[.])?\d+$/);
  },
  isAlpha(values, value) {
    return validation.matchRegexp(values, value, /^[A-Z]+$/i);
  },
  isAlphanumeric(values, value) {
    return validation.matchRegexp(values, value, /^[0-9A-Z]+$/i);
  },
  isInt(values, value) {
    return validation.matchRegexp(values, value, /^(?:[-+]?(?:0|[1-9]\d*))$/);
  },
  isFloat(values, value) {
    return validation.matchRegexp(values, value, /^(?:[-+]?(?:\d+))?(?:\.\d*)?(?:[eE][\+\-]?(?:\d+))?$/);
  },
  isWords(values, value) {
    return validation.matchRegexp(values, value, /^[A-Z\s]+$/i);
  },
  isSpecialWords(values, value) {
    return validation.matchRegexp(values, value, /^[A-Z\s\u00C0-\u017F]+$/i);
  },
